...
class CNN_TFClassifier():
	...
	def __init__(self, input_shape, num_outputs, model_dir='cnn_model', first_conv=[3, 3, 1, 32], second_conv=[3, 3, 32, 64],\
		num_epochs=50, batch_size=128, learning_rate=1e-4, dropout_prob=0.5, patience=5, output_activation='sigmoid',
	...
	def prepare_graph(self):
		...
		with tf.name_scope("conv1"):
			W_conv1 = self.weight_variable(self.first_conv, name="weight")
			b_conv1 = self.bias_variable([self.first_conv[3]], name="bias")
		...
		with tf.name_scope("conv2"):
			W_conv2 = self.weight_variable(self.second_conv, name="weight")
			b_conv2 = self.bias_variable([self.second_conv[3]], name="bias")
		...
		with tf.name_scope("fc1"):
			W_fc1 = self.weight_variable([new_width * new_height * self.second_conv[3], num_fully_connected], name="weight")
			b_fc1 = self.bias_variable([num_fully_connected], name="bias")
		...
		with tf.name_scope("fc2"):
			W_fc2 = self.weight_variable([num_fully_connected, self.num_outputs], name="weight")
			b_fc2 = self.bias_variable([self.num_outputs], name="bias")
