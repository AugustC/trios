...
*/
    ...
    int                         /*+ Purpose: Computes the number of bits ON in a w-pattern + */
     wpat_dimension(unsigned int *wpat, int wzip        /*+ In: w-pattern compacted size                    + */
        )
...
*/
    ...
    int                         /*+ Purpose: verify the order relation between two w-patterns + */
     wpat_order(unsigned int *wpat1, unsigned int *wpat2, int wzip      /*+ In: compacted w-pattern size                    + */
        )
...
*/
    ...
    int                         /*+ Purpose: verify whether a given w-pattern is greater than
                                   another one                                      + */
     wpat_greater_than(unsigned int *wpat1, unsigned int *wpat2, int wzip       /*+ In: compacted w-pattern size                    + */
        );
...
**/
    ...
    int *                       /*+ Purpose: create the offset vector according to window size + */
     offset_create(int wsize    /*+ In: window size + */
        );
/*+ Return: Pointer to offset vector if successful or NULL in case of
    ...
    void                        /*+ Purpose: set the offset vector according to a window and an image + */
     offset_set(int *offset,    /*+ In/Out: the offset vector                         + */
...
*/
    ...
    int                         /*+ Purpose: compare two w-patterns (taken as integers)    + */
     wpat_compare(unsigned int *wpat1, unsigned int *wpat2, int wzip    /*+ In: compacted w-pattern size                    + */
        );
...
*/
    ...
    int                         /*+ Purpose: compare two XG w-patterns                     + */
     wpat_compareXG(int *wpat1, /*+ In: first w-pattern                             + */
...
*/
    ...
    int                         /*+ Purpose: computes the Hamming distance between two
                                   binary w-patterns                                      + */
     wpat_hamming(unsigned int *wpat1, unsigned int *wpat2, int wzip    /*+ In: compacted w-pattern's size                  + */
        );
...
*/
    ...
    int                         /*+ Purpose: calculate the size of compacted w-pattern + */
     size_of_zpat(int wsize     /*+ size of w-pattern                                  + */
        );
/*+ Return: the compacted w-pattern size                                 +*/

    freq_node *                 /*+ Purpose: Creates a node for the frequency list + */
     freq_node_create(int label,        /*+ In: label of the pattern                       + */
                      unsigned int freq /*+ In: frequency of the pattern                   + */
        );

    void
     freqlist_free(freq_node * freq_list        /*+ In: frequency list to be free          + */
        );

    int
     set_freq(freq_node * freqlstin,    /*+ In: pointer to the input frequency list  + */
...
*/
    ...
    unsigned int                /*+ Purpose: Sums the frequencies of w-pattern             + */
     freq_sum(freq_node * freqlist      /*+ In: frequency list of a w-pattern         + */
        );
